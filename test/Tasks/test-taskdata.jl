...
function _create_policy_dict(params_dict::Dict)::Dict
	POLICY_DICT = convert(Dict{String, Any}, Dict("kind" => "Policy", "params" => params_dict))
	return POLICY_DICT
end

function _create_simulation_policy_path_dict(load::Bool, path::String, format::Dict{String, Any})::Dict
	POLICY_PATH_DICT = convert(Dict{String, Any},
		Dict(
			"load" => load,
			"path" => path,
			"format" => format,
...
function _create_simulation_dict(params_dict::Dict)::Dict
	SIMULATION_DICT = convert(Dict{String, Any}, Dict("kind" => "Simulation", "params" => params_dict))
...
SIMULATION_POLICY_DICT = _create_simulation_policy_path_dict(false, "out/policy", Dict("kind" => "AnyFormat", "params" => Dict()))
SIMULATION_PARAMS_DICT = _create_simulation_params_dict(SIMULATION_POLICY_DICT)

# Creating Tasks dict
TASKS_DICT = convert(Dict{String, Any}, Dict("tasks" => [_create_policy_dict(POLICY_PARAMS_DICT), _create_simulation_dict(SIMULATION_PARAMS_DICT)]))
...
@testset "task-taskdata" begin
	...
	@testset "task-valid-simulation-policy" begin
		d_simulation_policy = _create_simulation_policy_path_dict(true, "out/policy", Dict("kind" => "AnyFormat", "params" => Dict()))
		d_simulation = _create_simulation_dict(_create_simulation_params_dict(d_simulation_policy))
	...
	@testset "task-invalid-simulation-policy" begin
		d_simulation_policy = _create_simulation_policy_path_dict(false, "policy", Dict("kind" => "AnyFormat", "params" => Dict()))
		d_simulation = _create_simulation_dict(_create_simulation_params_dict(d_simulation_policy))
