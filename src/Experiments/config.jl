...
function read_experiment_config(path::String)::ExperimentConfig
    ...
    if !haskey(d, "configurations")
        push!(e, ErrorException("experiment.jsonc is missing required key \"configurations\""))
    end
    !isempty(e) && throw(e)
    ...
    base_study_path = isabspath(raw_base) ? raw_base : normpath(joinpath(experiment_dir, raw_base))
    ...
    if !isfile(joinpath(base_study_path, "main.jsonc"))
        push!(e, ErrorException("base_study directory \"$base_study_path\" does not contain a main.jsonc"))
    end
    ...
    output_dir = isabspath(raw_output) ? raw_output : normpath(joinpath(experiment_dir, raw_output))
    ...
    if !(raw_configs isa Dict)
        push!(e, ErrorException("\"configurations\" must be a Dict, got $(typeof(raw_configs))"))
        throw(e)
    end
    ...
    for (name, overrides) in raw_configs
        ...
        if !(overrides isa Dict)
            push!(e, ErrorException("Configuration \"$name\" must be a Dict, got $(typeof(overrides))"))
            continue
        end
