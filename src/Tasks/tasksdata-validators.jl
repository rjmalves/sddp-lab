...
function __validate_simulation_task_policy_load_path(
    ...
    valid_simulation_policy_path =
        (policy_save == true) & (simulation_policy_path == policy_path)
...
function __validate_simulation_task_with_policy_task(
    tasks::Vector{TaskDefinition}, e::CompositeException
)::Bool
    ...
    policy_index = findfirst(x -> isa(x, Policy), tasks)
    valid_simulation_policy =
        (policy_index !== nothing) && (simulation_index > policy_index)
    valid_simulation_policy || push!(
        e,
        ErrorException(
            "Either needs to define a policy task prior to simulation task or load a policy.",
        ),
    )
...
function __validate_simulation_task_policy(
    ...
    valid = if simulation.policy.load
        __validate_simulation_task_policy_load_path(tasks, e)
    else
        __validate_simulation_task_with_policy_task(tasks, e)
    end
    return valid
end
...
function __validate_tasksdata_consistency!(d::Dict{String,Any}, e::CompositeException)::Bool
    ...
    valid_simulation_policy = if simulation_index != nothing
        __validate_simulation_task_policy(task_vector, e)
    else
        true
    end
