...
function __validate_markov_chain_keys_types!(
    ...
    valid_types =
        valid_keys && __validate_key_types!(d, MARKOV_CHAIN_KEYS, MARKOV_CHAIN_KEY_TYPES, e)
    return valid_types
end

function __validate_transition_matrices!(d::Dict{String,Any}, e::CompositeException)::Bool
    ...
    for (i, raw_mat) in enumerate(raw_matrices)
        if !(raw_mat isa Vector)
            push!(
                e,
                AssertionError(
                    "transition_matrices[$i] must be a matrix (Vector of rows), got $(typeof(raw_mat))",
                ),
            )
            valid = false
            continue
        ...
        for (j, row) in enumerate(raw_mat)
            if !(row isa Vector)
                push!(
                    e,
                    AssertionError(
                        "transition_matrices[$i] row $j must be a Vector, got $(typeof(row))",
                    ),
                )
                row_valid = false
                continue
            ...
            elseif length(row) != ncols
                push!(
                    e,
                    AssertionError(
                        "transition_matrices[$i] row $j has $(length(row)) columns, expected $ncols",
                    ),
                )
                row_valid = false
            end
            for (k, val) in enumerate(row)
                if !(val isa Real)
                    push!(
                        e,
                        AssertionError(
                            "transition_matrices[$i][$j][$k] must be a Real, got $(typeof(val))",
                        ),
                    )
                    row_valid = false
                end
    ...
    for i in 2:length(matrices)
        ...
        if nrows != ncols
            push!(
                e,
                AssertionError("transition_matrices[$i] must be square ($nrows x $ncols)"),
            )
            valid = false
        end
    end
