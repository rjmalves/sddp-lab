...
function plot_simulation_results(
    ...
    # termo e sistema
    min_generation = sum(x.min_generation for x in cfg.thermals.entities)
    max_generation = sum(x.max_generation for x in cfg.thermals.entities)
    SDDP.add_spaghetti(
        plt; title = "GT", ymin = min_generation, ymax = max_generation
    ) do data
        return sum(data[:gt])
    end
